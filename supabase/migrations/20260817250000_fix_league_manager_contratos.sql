-- Director de esports = league_manager (Manager Liga), no manager genérico.
...
CREATE POLICY documents_org_read ON public.documents
  ...
  USING (
    ...
    AND (
      ...
      OR (
        category = 'Contratos'
        AND private.has_role(ARRAY['owner','league_manager','assistant']::public.app_role[])
      )
      OR (
        kind IS NULL
        AND category IS DISTINCT FROM 'Contratos'
...
CREATE POLICY documents_org_write ON public.documents
  ...
  USING (
    ...
    AND (
      ...
      OR private.has_role(ARRAY['owner','league_manager','assistant']::public.app_role[])
    )
  )
  WITH CHECK (
    organization_id = private.current_org_id()
